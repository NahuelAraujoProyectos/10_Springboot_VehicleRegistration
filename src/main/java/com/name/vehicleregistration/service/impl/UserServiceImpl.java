...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public String getImageMimeType(Integer id) {
        UserEntity userEntity = userRepository.findById(id)
                .orElseThrow(() -> new UserNotFoundException("User not found with ID: " + id));
        return userEntity.getImageType();
    }
    ...
    public void addUserImage(Integer id, MultipartFile file) {
        ...
        if (mimeType == null || !(mimeType.equals(MediaType.IMAGE_PNG_VALUE) || mimeType.equals(MediaType.IMAGE_JPEG_VALUE))) {
            throw new InvalidImageFormatException("Unsupported image format. Only PNG and JPEG are allowed.");
        }
        ...
        try {
            ...
            userEntity.setImageType(mimeType);
            userRepository.save(userEntity);
            log.info("POST -> Image saved in User with id {}.", id);
        } catch (IOException e) {
            log.error("Error saving image for user with id {}: {}", id, e.getMessage());
