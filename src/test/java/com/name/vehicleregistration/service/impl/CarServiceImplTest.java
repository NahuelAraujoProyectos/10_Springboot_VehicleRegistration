...
class CarServiceImplTest {
    ...
    void uploadCars_success() throws Exception {
        ...
        // Then
        assertEquals("1 cars have been uploaded and saved from the CSV file.", result);
    }
    ...
    void uploadCars_ko_brandNotFound() throws Exception {
        ...
        assertEquals("Brand not found: Ford", exception.getMessage());
    }
    ...
    void uploadCars_ko_invalidFileType() {
        ...
        assertEquals("The file is not a valid CSV.", exception.getMessage());
    }
    ...
    void uploadCars_fail_ioException() throws IOException {
        ...
        when(file.getInputStream()).thenThrow(new IOException("Could not read CSV file"));

        // When & Then
        CsvFileException exception = assertThrows(CsvFileException.class, () -> carService.uploadCars(file));
        assertEquals("Could not read CSV file", exception.getMessage());
